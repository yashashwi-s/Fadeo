...
- **Auto-update:** [Sparkle](https://sparkle-project.org) was wired then **removed** — on
  macOS 26/27 beta, merely creating `SPUStandardUpdaterController` (at any time, eager or
  deferred) breaks the main window at the AppKit level: it renders and takes clicks but
  exposes zero accessibility and its `ScrollView`s stop scrolling entirely (root-caused by
  bisection against a minimal control app + the AX tree). Revisit auto-update with a
  different integration (custom `SPUUserDriver`, or a stable-macOS retest) before shipping.
